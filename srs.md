# Smart Parking Slot Occupancy Detection and Shortest-Path Navigation — A system for parking slot occupancy detection and shortest‑path navigation using direct image analysis

# Context description and motivation (no more than 5 sentences)
In indoor parking lots (shopping malls), drivers often spend 5–15 minutes cruising for a vacant space because real‑time occupancy information is unavailable. Unnecessary cruising wastes time, increases internal congestion, and degrades the user experience. The proposed system uses direct image analysis (edge density, color variance, texture energy) to classify the occupancy status (occupied/vacant) of pre-defined parking slots, and applies a shortest‑path algorithm on a graph to guide the driver to the nearest vacant slot. The system is designed to run efficiently on standard CPU hardware, ensuring feasibility for academic research and demo environments without requiring expensive GPUs.

# Input
1. **Surveillance camera video**: Pre‑recorded video or a video stream (RTSP/USB), minimum resolution 720p (1080p recommended), frame rate ≥ 15 FPS. The camera observes an area of ≤ 20 parking slots. The input video is processed frame by frame by extracting specific regions defined by the slot polygons.
...
3. **Current vehicle position**: The user selects a starting node on the web interface (e.g., entrance A, entrance B) or clicks directly on the video feed to set their position for the path‑finding algorithm.
...
|---|---|---|
| R1 | Accurately classify the status of each parking slot (vacant / occupied). Pipeline: Crop slot polygon → compute edge density, color variance, texture energy → combine with thresholds → if score ≥ threshold → occupied, otherwise = vacant. | Slot‑level F1‑score ≥ 0.90 on the test set |
...
| R3 | End‑to‑end processing time: from receiving 1 frame → analyze slots → find path → return result. Measured on standard CPU hardware. | ≤ 2 seconds (average over 50 trials) |
...
2. **Model**: The system uses a direct per-slot image analysis approach utilizing classical computer vision techniques (Canny edge detection, Laplacian variance, color standard deviation). It does not use deep learning models like YOLO, eliminating the need for a GPU and model training/fine-tuning.
...
4. **Processing hardware**: Standard CPU hardware (e.g., Intel Core i5 or equivalent). A GPU is not required.
5. **No fine‑tuning required**: Since the system relies on tunable thresholds for classical image features, there is no need for bounding-box annotations or deep learning model training. Thresholds can be adjusted via the configuration file if necessary.
...
8. The standard CPU hardware is available throughout the demo/evaluation.
9. Classical CV features are sufficient to distinguish between empty pavement and occupied parking slots.

# Scope
**In‑scope:**
- Extract visual features (edges, texture, color) from slot polygons → infer parking slot status (vacant/occupied) based on threshold logic.
...
- The entire inference pipeline runs purely on the CPU.
...
- **Evaluation pipeline**:
  ...
  2. **System prediction**: The system extracts the slot polygon from the frame, computes visual features (edge density, color variance, texture energy), and checks the combined score against a predefined threshold → if score ≥ threshold → slot = occupied, otherwise = vacant.
...
- **Reason for slot-level metric**: The core problem is occupancy classification at the slot level, so the correct metrics are Precision/Recall/F1 on the slot labels, instead of evaluating bounding boxes.
...
- **Measurement method**:
  ...
  - Report alongside: CPU configuration, number of slots in the lot, frame size.
- **Threshold rationale**: Direct image analysis per-slot is extremely fast. Processing 20 slots using classical CV techniques on a standard CPU typically takes under 50ms. An end‑to‑end threshold of 2 seconds (including rendering + web transmission) is very achievable.
...
|---|---|---|
| TC5.1 | Annotated video displays in the browser | Video renders continuously, slot polygons show correct colors (green/red), no stutter > 2s |
| TC5.2 | 2D diagram updates when slot status changes | Status on diagram matches annotated video within 5s |
| TC5.3 | User selects a starting position (or clicks video) and the system displays the route | Route is displayed on the 2D diagram within 5s after selection |
...
| R3 – Response time (on CPU) | M3 – Avg Response Time | ≤ 2s | ≥ 50 trials |
