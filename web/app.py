...
Now includes an interactive annotation tool for new parking lots.
"""
...
def load_system():
    """Load the parking system configuration and initialize pipeline."""
    ...
    # Check if a custom video path is defined in config
    if "video_path" in config and config["video_path"]:
        ...
        if os.path.exists(v_path):
            ...
            if total_video_frames <= 0: 
                total_video_frames = 300 # Fallback
            video_path = v_path
            frame_pairs = []
    ...
    # Fallback to PKLot frame sequence
...
def extract_frame():
    ...
    # Extract frame (skip first 100 frames to avoid black intros)
    if path.lower().endswith(('.mp4', '.avi', '.mov', '.mkv')):
        cap = cv2.VideoCapture(path)
        cap.set(cv2.CAP_PROP_POS_FRAMES, 100)
        ret, frame = cap.read()
        if not ret:
            # Fallback to first frame if video is short
...
def save_config():
    ...
    # Connect slots to nearest waypoint
    for slot in slots:
...
def set_video():
    ...
    with video_lock:
        ...
        current_frame_idx = 0
        frame_pairs = [] # Clear frame pairs since we are using video
        
    # Also update config file so it persists
    ...
    return jsonify({
        "success": True, 
        "message": f"Video set to {new_video_path}",
...
def get_config():
    """Return parking lot configuration (slots, graph)."""
    if config is None:
...
def process_single_frame():
    ...
    if video_cap is not None:
        ...
        img_path, xml_path = frame_pairs[current_frame_idx]
...
def next_frame():
    """Advance to next frame and process."""
...
def set_start_node():
    """Set the starting node for navigation."""
...
def handle_start_stream():
    """Start streaming frames via WebSocket."""
    ...
    while is_playing:
        ...
        socketio.sleep(0.5)  # ~2 FPS streaming
